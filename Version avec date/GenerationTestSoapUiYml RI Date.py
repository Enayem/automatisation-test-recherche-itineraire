...
from Structure_TestSuite_SMMV import HeadGlobal, testSuiteHead, testSuiteBottom, BottomGlobal,   testStepHead, testStepBottom, testCaseHead, testCaseBottom
from proxy import proxies


parser = argparse.ArgumentParser(description="Cet outils permet de générer un fichier Yml et/ou un projet SoapUI à partir d'un fichier csv contenant des RI \n\nL'en-tête du fichier csv attendu :Indice, AdressStart, AdressEnd, startFrom,withTrafficEvents, profil, preferNetwork [Multiple séparé par virgule], date, AssertionImpactPerturbation  ")
...
# if args.dateRI :
dateRI = datetime.strptime("2016-11-21T09:00:00"+"+0100", "%Y-%m-%dT%H:%M:%S%z")
...
if(args.soapUiProject):
    ...
    print("""#Prise en compte des date dans le fichier Csv"                                         #""")
    ...
    with codecs.open(nomFichierSortie, 'w', 'utf8') as fichierSortie:
        ...
        fichierSortie.write (testCaseHead.format(TestSuiteName = args.nomFichier , uuid2=uuid.uuid1()))   
        ...
        with open(args.csvPath,  newline='', encoding='latin-1') as csvfile:
          ...
          for ri in CSVRI:
                ...
                print(ri['date'])
                dateRI = datetime.strptime(ri['date']+"+0100", "%Y-%m-%dT%H:%M:%S%z")
...
# <con:assertion type="XPath Match" id="c05d6e4f-717c-432e-a017-2d5312e67572" name="XPath Match">
    # <con:configuration>
        # <path>declare namespace ns1='http://smartmove.ixxi/';\n
...
# /SOAP-ENV:Envelope/SOAP-ENV:Body/ns1:getItineraryResponse/return/itineraries/impactPerburbation</path>
...
# </con:assertion>
                ...
                #Création de l'assertion pour vérifier la valeur contenue dans l'impact perturbation du résultat de RI
                fichierSortie.write ("""  <con:assertion type="XPath Match" id="{}" name="Impact Perturbation"> """.format(uuid.uuid1()))
                fichierSortie.write ("""    <con:configuration>""")
                fichierSortie.write ("""        <path>declare namespace ns1='http://smartmove.ixxi/'; """)
                fichierSortie.write ("""              declare namespace SOAP-ENV='http://www.w3.org/2003/05/soap-envelope';""")
                fichierSortie.write ("""              declare namespace SOAP-ENC='http://www.w3.org/2003/05/soap-encoding'; """)
                fichierSortie.write ("""              /SOAP-ENV:Envelope/SOAP-ENV:Body/ns1:getItineraryResponse/return/itineraries/impactPerburbation</path>""")
                fichierSortie.write ("""        <content>{}</content>""".format(ri['AssertionImpactPerturbation']))
                fichierSortie.write ("""        <allowWildcards>false</allowWildcards>""")
                fichierSortie.write ("""        <ignoreNamspaceDifferences>false</ignoreNamspaceDifferences>""")
                fichierSortie.write ("""        <ignoreComments>false</ignoreComments>""")
                fichierSortie.write ("""   </con:configuration>""")
